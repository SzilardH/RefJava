...
class PatternParser {
	...
	def static parse(String patternString) {
		if (!initialized) {
			resourceSet = (new RefJavaStandaloneSetup().createInjectorAndDoEMFRegistration()).getInstance(typeof(XtextResourceSet))
			resourceSet.addLoadOption(XtextResource.OPTION_RESOLVE_ALL, Boolean.TRUE)
			resource = resourceSet.createResource(URI.createURI("dummy:/patterns.refjava"))
			initialized = true
			println("ASDASD22")
		...
		println(refact.matchingPattern)
		//resource.delete(newHashMap)
